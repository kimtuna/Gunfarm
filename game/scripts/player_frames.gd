...
## 칸 안에서 **인물이 실제로 차지하는 키**(아트 px). **`CELL` 과 다른 값이다** —
## 2026-09-10 에 인물이 96 → 72px 로 내려가면서 칸(96)에 24px 이 남았고, 그 여유가
## 가로로 넓은 도구를 받는다(docs/DESIGN.md 「아이템/오브젝트 크기 표준」).
## 값의 원본은 `game/tools/gen_player.py` 의 `FIG` 이고 여기는 그것을 옮겨 적은 것이다 —
## **어긋나면 `qa_player_world.gd` 가 시트를 실제로 재서 잡는다.**
const FIGURE := 72

## 발밑에서 **몸 한가운데**까지의 높이(월드 단위 = 화면 px, 배율 1인 96px 시트 기준).
## 조준 각도를 재는 기준점(`player.gd` 의 `aim_origin()`)이자 총알이 그려지는 높이
## (`bullets_view.gd` 의 `LIFT`)다. **둘이 같은 값이라야 총알이 조준선에서 나간다.**
##
## **2026-09-10 (INBOX #76) 에 「칸의 절반」에서 「인물 키의 절반」으로 고쳤다.**
## 그전에는 `CELL * SCALE * 0.5` = 48 이었는데, 그게 맞았던 것은 **인물이 칸을 꽉 채우던
## 동안**뿐이다 — 인물이 72px 로 내려가자 48 은 몸 한가운데(36)가 아니라 목 언저리가
## 됐고, 총알과 예광탄이 **총이 아니라 머리 옆에서** 날아갔다(실측: 총이 있는 줄은
## 발밑 위 38px, 몸 한가운데는 36.5px, 코드가 쓰던 값은 48px).
const BODY_CENTER := FIGURE * 0.5
...
##
## **32px 시트만 다르다**(2026-09-08, INBOX #47). 그 칸의 idle 은
