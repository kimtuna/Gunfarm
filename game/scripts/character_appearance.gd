...
## **2026-09-08 — 지금은 「크기 비교」용 두 개뿐이다.**
## 사람이 48px 과 96px 을 인게임에서 나란히 보고 고르기로 했다. 캐릭터 시트를 전부
## 지우고 ComfyUI 도안 하나에서 두 크기로 구웠기 때문에, 머리모양 종류가 아직 없다.
## 크기를 정하고 나면 여기가 다시 진짜 머리모양 목록이 된다.
## **지금은 한 벌뿐이다** (2026-09-08). 캐릭터를 ComfyUI 그림에서 만들기로 하면서
## 손으로 찍은 48px/96px 두 벌을 버렸다 — 사람이 보고 물렸다. 머리모양이 늘면
## 여기에 줄을 늘리고 같은 이름의 시트를 `assets/sprites/` 에 넣는다
## (`player_frames.sheet_path()`).
const HAIRSTYLE := [
	{"id": "farmer", "label": "농부"},
]
