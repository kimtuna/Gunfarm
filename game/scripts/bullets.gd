...
## PvE 라 사람도 안 맞는다 (docs/DESIGN.md 「전투」의 2026-09-07 결정). 그래서 지금
## 총알이 사라지는 길은 **아래 `blocks_bullet` 에 걸리거나 사거리를 다 쓰거나** 둘뿐이다.
##
...
## **총알을 막는 것이 꽂히는 자리다.** 게임에서는 `world.gd` 가 여기에 **나무·바위**를
## 꽂는다 (2026-09-08, INBOX #65 — `world_gen.gd` 의 `blocks_bullet_at()`). 덤불은
## 키가 없어 안 막고, 물도 안 막는다 (docs/DESIGN.md 「월드 오브젝트」의 「막는 것과
## 막는 크기」). **아무것도 안 꽂으면 총알은 사거리를 다 날아가야만 사라진다** —
## 그 상태도 고장이 아니다(씬 없이 코어만 돌릴 때가 그렇다).
##
...
##
## 키가 있는 오브젝트(벽·문 등)를 만드는 바퀴가 여기에 꽂는 것도 같은 모양
## (`func(point: Vector2) -> bool`)이다 — "이건 키가 있으니 막는다"를 그 오브젝트
## 쪽에 한 줄 적는 것이 전부다.
