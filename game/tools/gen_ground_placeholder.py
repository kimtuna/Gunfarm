"""바닥 자리표시 더미의 무늬를 찍는다 (docs/INBOX.md #70).
...
CANDIDATES = {
    # **채택한 것** — 납작한 덩어리 넷을 어긋나게 쌓는다. 갈린 자리는 「덩어리가
    # 몇 개인가」가 아니라 **덩어리가 동그란가 납작한가**였다: 정원(正圓)에 가까운
    # 후보는 크기를 아무리 달리해도 **대포알을 쌓은 것**으로 읽혔고, 가로로 눌러야
    # 「퍼내 쌓은 재료」가 된다. 넷을 좌우로 어긋나게 놓아 좌우 대칭도 깬다.
    "stack": [(11.0, 24.5, 10.0, 6.5), (25.0, 24.0, 9.5, 7.0),
              (16.0, 16.0, 10.0, 6.5), (20.0, 8.5, 8.5, 7.5)],
    # 동그란 것 셋(위가 한 뼘 작다). 깔끔하지만 **대포알 세 개**로 읽힌다.
