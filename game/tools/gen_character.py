...
CFG = dict(
    ...
    # ── 걷기(walk) ── **아래 값들은 아직 34px 단위다.** 17px 에서 그대로 쓰면 한
    # 걸음이 몸 폭만큼 벌어진다 — 걷기 시트를 다시 굽는 INBOX #20 이 여기를 잡는다.
    # idle 은 `walk_pose(phase=None)` 이라 이 값들을 하나도 안 쓴다.
...
def _body(b, d, cfg, pose):
    ...
    for i, offx in enumerate(xs):
        ...
        arm_top, arm_bot = cfg["torso_top"] + 0.55 + ay, cfg["torso_bot"] - 1.0 + ay
...
# 이번에 다시 굽는 모션. **걷기는 아직 없다**(2026-09-07, INBOX #19) — `CFG` 의
# 걷기 진폭이 34px 단위라 17px 에서 그대로 구우면 한 걸음이 몸 폭만큼 벌어진다.
# 걷기 시트 4장은 34px 인 채로 남아 있고 **INBOX #20 이 여기에 `"walk"` 를 도로
# 넣으면서** 다시 굽는다.
MOTIONS_NOW = (("idle", idle_sheet),)
