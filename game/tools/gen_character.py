...
def _body(b, d, cfg, pose):
    ...
    for i, offx in enumerate(xs):
        ...
        # 팔뚝**이 보인다: 참고 자료와 우리 것을 나란히 놓았을 때 가장 먼저
...
def _hair(b, d, cfg, hx, hy, rx, ry, style, bob=0.0):
    ...
    **덧붙이는 갈래는 전부 머리 껍데기(shell) 안으로 잘라 넣는다** — 안 그러면
    ...
    if st["strand"]:
        ...
        # 참고 자료와 나란히 놓았을 때 우리 머리가 "매끈한 돔"이던 이유가
...
def character(direction="down", hair="short", pal=None, cfg=None, phase=None,
    ...
    EYE_ART = {
        ...
        # **2×2 인데 바깥 칸이 흰자다** (2026-09-08, INBOX #47). 아트 2칸 폭에
        # **바깥이 흰자 · 안쪽이 눈동자**로 놓는다 — 우리가 쓰던
...
# ── 아이콘 칸은 51px 이다 — **도트 하나 = 화면 1px** (2026-09-08, INBOX #67) ────
# 그전에는 17px 칸을 인벤토리에서 3배로 늘려 그렸다(화면 51px). 배율은 정수였지만
# **아트 픽셀 하나가 화면에서 3px** 이라, 도트가 1px 인 캐릭터(96px 칸 × 1배)·지형
# (48px 칸 × 1배) 옆에 놓으면 도구만 뭉툭했다 — `docs/STYLE_GUIDE.md` 1번의
# *"배율이 이웃과 다르면 정수여도 어긋난다"* 가 이번에는 **UI 칸 안**에서 걸린 것이다.
#
# **화면 크기는 한 픽셀도 안 바뀐다**(51px 그대로): 칸(`inventory_panel.gd` 의 `SLOT`
# 56px)도, 「아이템/오브젝트 크기 표준」의 32px 하한도 그대로다. 바뀌는 것은
# **같은 그림을 몇 칸에 찍는가**뿐이라, 아래 `ICONS` 의 길이 값·각도·손잡이 자리를
# 한 줄도 안 고친다(`canvas_scale()` 이 설계 공간 17칸을 그대로 두고 격자만 촘촘히
# 깐다 — 캐릭터 캔버스를 키울 때 쓰던 그 함수다).
ICON_N = 51


def tool_icon(tool="axe", pal=None):
    """인벤토리 칸에 보일 도구 아이콘 한 장(`ICON_N` px).
    ...
    **월드의 도끼와 같은 파이프라인으로 그린다** — 같은 램프, 같은 광원(왼쪽 위),
    ...
    화면에서는 **배율 1**로 그린다 — 아트 픽셀 하나가 화면 1px 이라 캐릭터·지형과
    도트 결이 같다(위 `ICON_N`).
    """
    spec = ICONS[tool]
    pal = pal or palette()
    with canvas_scale(ICON_N):
        b = Build()
        draw_tool(b, spec["grip"][0], spec["grip"][1], spec["angle"], 1.0, spec["kit"],
                  lift=0.0)
        lum = light(b.hgt, b.mat, key=CFG["key"], amb=CFG["amb"], rim=CFG["rim"])
        m, l = downsample(b.mat, lum)
        pm = downsample_part(b.part)
...
#
# 그래서 **더 작은 칸에 따로 굽는다.** 화면 36px 이라 예전(34px)과 거의 같은
# 크기인데 도트 결만 월드와 같아진다.
#
...
#
# **2026-09-08 (INBOX #67) — 그 12칸을 36px 에 찍는다.** 화면 크기(36px)와 도구가
# 칸에서 차지하는 비율은 그대로이고, 바뀌는 것은 **도트 하나가 화면 3px 에서 1px 로**
# 잘아진 것뿐이다(아이콘과 같은 자리, 위 `ICON_N`). 그래서 아래 `GROUND_K` 는 여전히
# **아이콘 칸에 대한 비**(36/51 = 12/17)이고 `shrink()` 가 주는 형태도 그대로다 —
# 「10/11/12 중 12」라는 위 판단이 그대로 살아 있다.
GROUND_N = 36

# 아이콘 칸에서 바닥 칸으로 줄이는 배율. **그림을 다시 정의하지 않고 같은
...
GROUND_K = GROUND_N / float(ICON_N)
...
GROUNDS = {
    ...
    # **낚싯대의 완화는 2026-09-08 (INBOX #67) 에 없앴다.** 아트가 12px 이던 시절에는
    # 릴(반지름 0.44px)이 축소 격자에서 한 칸도 못 채워 통째로 사라졌고(그러면 낚싯대가
    # 그냥 휜 나뭇가지다), 1px 굵기의 대만 남아 밝은 윗면과 그늘이 한 톤으로 평균나서
    # 「명암폭」이 34 까지 떨어졌다 — 그래서 릴을 1.9배, 손잡이를 1.2배로 굵혔었다.
    # **칸이 36px 이 되면서 두 문제가 같이 사라졌다**(릴 1.3px 반지름, 명암폭 73).
    # 굵힌 채로 두면 이번에는 반대로 **바닥의 릴만 아이콘보다 커서** 「같은 도구가
    # 사는 자리는 셋」이 깨진다(실제로 나란히 놓고 확인했다).
    # 낫의 `grip_shift` 는 남는다 — 그건 해상도가 아니라 **날이 자루보다 위로 넓다**는
    # 형태의 문제라, 36px 에서도 빼면 날 끝이 칸 위 테두리에 두 칸 닿는다(실측).
}


def ground_icon(tool="axe", pal=None):
    """바닥에 떨어진 도구 한 장(`GROUND_N` px). 화면에서는 **배율 1**로 그린다.
    ...
    **줄이기만 해서는 안 되는 자리는 `GROUNDS` 에만 적는다.**
    ...
    # **설계 공간은 아이콘의 17칸을 `GROUND_K` 로 줄인 것**이고, 그것을 `ICON_N/17`
    # 배(=3)로 촘촘한 격자에 찍는다 — 그림은 그대로, 도트만 잘아진다(위 `GROUND_N`).
    with canvas(GROUND_N, ICON_N / float(DESIGN_N)):
        b = Build()
...
# **머리 40% / 1 : 2** 를 목표로 했는데, 이 참고 자료는 그것과 두 군데가
