...
## 비슷한 색을 고르면 그게 눈에도 검사에도 안 잡힌다.
const CHOSEN := {
	"skin": "deep", "hair_color": "silver", "clothes_color": "plum", "hairstyle": "ponytail",
}
...
func _check_hairstyle_sheet() -> void:
	var chosen: Texture2D = load(PlayerFrames.sheet_path("idle", String(CHOSEN["hairstyle"])))
	var default_sheet: Texture2D = load(PlayerFrames.sheet_path("idle", PlayerFrames.DEFAULT_HAIRSTYLE))
	if chosen == null or default_sheet == null:
		_fails.append("머리모양 시트를 못 읽었다 — `--import` 를 안 돌렸을 수 있다")
		return
	var chosen_image := chosen.get_image()
