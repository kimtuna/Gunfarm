...
## 키를 누르고 있는 동안 그 방향의 걷기가 돌고, **프레임이 실제로 넘어가는가.**
func _check_walking(dir: String) -> void:
	...
	if sprite.animation != want:
		_fails.append("%s 키를 누르고 있는데 애니메이션이 %s 다" % [dir, sprite.animation])
		return
...
func _check_idle(dir: String) -> void:
	...
	if sprite.animation != "idle_%s" % dir:
		_fails.append("%s 키를 뗐는데 애니메이션이 %s 다 — idle 로 안 돌아온다"
				% [dir, sprite.animation])


# --- 조작 ---------------------------------------------------------------------

## `Input.action_press()` 로 실제 입력처럼 눌러준다 — `player.gd` 는 키만 읽으므로
## (docs/DESIGN.md 「서버 권위」) 여기를 통과시키면 실제 조작과 같은 경로가 돈다.
func _press(dir: String) -> void:
	Input.action_press(KEYS[dir])
	_seen_frames[dir] = 0
	_wait = HOLD_FRAMES + 5
...
func _report() -> bool:
	if _fails.is_empty():
		print("[qa] PASS — 네 방향 걷기가 실려 있고, 외형대로 칠해지고, 이동 키에 맞춰 돈다")
		return true
	for f in _fails:
